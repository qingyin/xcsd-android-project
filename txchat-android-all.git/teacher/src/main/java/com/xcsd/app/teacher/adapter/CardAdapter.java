...
import com.xcsd.app.teacher.activity.HeyInfoActivity;
...
public class CardAdapter extends ArrayAdapter<CheckInRecord> {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			viewHolder.current_cardClassName = (TextView) convertView.findViewById(R.id.home_card_current_class);
		...
		if(currentStame.split(" ")[0].equals(timeStame.split(" ")[0])){
			...
			try {
				...
				if(array.length() > 0){
					...
					viewHolder.current_head.setImageUrl(url + SysConstants.Imgurlsuffix134, R.drawable.defal_down_proress,true);
				...
				viewHolder.current_cardName.setText(info.getUserName());
			...
			try {
				...
				viewHolder.week.setText(Utils.getWeekOfDate(timeStame.split(" ")[0], "yyyy年MM月dd日"));
					...
					if(array.length() > 0){
						final String url = array.getJSONObject(0).getString("url");
						viewHolder.head.setImageUrl(url + SysConstants.Imgurlsuffix134, R.drawable.defal_down_proress,true);
