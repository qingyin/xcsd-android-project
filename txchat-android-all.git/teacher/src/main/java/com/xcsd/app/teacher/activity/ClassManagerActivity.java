...
public class ClassManagerActivity extends BaseActivity {
    ...
    private void initView() {
        manager_jinyan_rl = (RelativeLayout) findViewById(R.id.manager_jinyan_rl);
        manager_jinyan_rl.setOnClickListener(this);
    }
            ...
            switch (event.getEvent()) {
                case DEPARTMENT_LOAD_FROM_LOCAL:
                    ...
                    if (department != null) {
                        ...
                        disProgressDialog();
                        if (department.getType()== com.tuxing.sdk.utils.Constants.DEPARTMENT_TYPE.CLASS &&
                                TuxingApp.VersionType.equals(com.tuxing.sdk.utils.Constants.VERSION.TEACHER.getVersion())) {
                            manager_jinyan_rl.setVisibility(View.VISIBLE);
                        }else
                            manager_jinyan_rl.setVisibility(View.GONE);
                    } else {
                        manager_jinyan_rl.setVisibility(View.GONE);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.manager_student_rl:
                ...
                intentParent.putExtra("title", "孩子家长");
            ...
            case R.id.manager_clear_msg:
                ...
                break;
            case R.id.manager_no_distrub_button:
                //TODO 消息免打扰
                idList.clear();
                if (distrub.isChecked()) {
                    EMChatManager.getInstance().getChatOptions().setReceiveNotNoifyGroup(null);
                    distrub.setChecked(false);
                    PreferenceUtils.setPrefBoolean(mContext, SysConstants.departmentdi_sturb, false);
                } else {
                    idList.add(String.valueOf(departmentId));
                    EMChatManager.getInstance().getChatOptions().setReceiveNotNoifyGroup(idList);
                    distrub.setChecked(true);
                    PreferenceUtils.setPrefBoolean(mContext, SysConstants.departmentdi_sturb, true);
                    MobclickAgent.onEvent(mContext,"class_manager_disturb",UmengData.class_manager_disturb);
                }
                break;
            case R.id.manager_jinyan_rl://禁言设置
...
//                finish();
    ...
    public void showData(Department data) {
        if(data.getType() == Constants.DEPARTMENT_TYPE.CLASS){
        ...
        classIcon.setImageUrl(data.getAvatar(), R.drawable.ic_class);
    ...
    public void onclickBtn1() {
       ...
       showToast("清除聊天记录成功");
        MobclickAgent.onEvent(mContext,"class_manager_clear_msg",UmengData.class_manager_clear_msg);
    	super.onclickBtn1();
    }
