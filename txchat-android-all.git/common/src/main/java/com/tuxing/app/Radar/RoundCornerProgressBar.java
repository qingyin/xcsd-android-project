...
//        layoutProgress.setLayoutParams(progressParams);
        ...
        float ratio = max / progress;
        int recent = (int) ((progress / max) * 100);
        int progressWidth = (int) ((totalWidth - (padding * 2)) / ratio);
        ViewGroup.LayoutParams progressParams = layoutProgress.getLayoutParams();

        progressParams.width = progressWidth;
        if (progress >= 5) {
            progressParams.height = DisplayUtils.dp2px(30);
        } else if (progress == 3) {
            progressParams.height = DisplayUtils.dp2px(24);
        } else if (progress == 4) {
            progressParams.height = DisplayUtils.dp2px(26);
        } else if (progress == 2) {
            progressParams.height = DisplayUtils.dp2px(20);
        } else {
            progressParams.height = DisplayUtils.dp2px(14);
        }
        layoutProgress.setLayoutParams(progressParams);
    }
