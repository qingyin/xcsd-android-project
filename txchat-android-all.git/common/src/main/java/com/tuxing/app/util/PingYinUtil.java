...
public class PingYinUtil {
	...
	public static String converterToFirstSpell(String chines) {
		...
		for (int i = 0; i < nameChar.length; i++) {
			if (nameChar[i] > 128) {
				try {
                    String[] pinyinNameArray = PinyinHelper.toHanyuPinyinStringArray(
                            nameChar[i], defaultFormat);

                    if(pinyinNameArray != null && pinyinNameArray.length > 0){
                        pinyinName += pinyinNameArray[0].charAt(0);
                    }
		...
		try {
			if (input.length>1){
				...
				if (java.lang.Character.toString(input[0]).
						matches("[\\u4E00-\\u9FA5]+")) {
					String[] temp = PinyinHelper.
							toHanyuPinyinStringArray(input[0],
									format);
					output += temp[0];
				} else
					output += java.lang.Character.toString(
