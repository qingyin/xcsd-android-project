...
public class GestureImageViewTouchListener implements OnTouchListener {
	...
	private boolean currentXBoundary = true;
	...
	public GestureImageViewTouchListener(final GestureImageView image, int displayWidth, int displayHeight) {
		...
		flingAnimation.setListener(new FlingAnimationListener() {
			...
			public boolean onMove(float x, float y) {
				 boolean b = handleDrag(current.x + x, current.y + y);
				 return b;
			}
			...
			public void onComplete() {
				 if(imageListener != null){
					 imageListener.onFling(flingListener.getE1(), flingListener.getE2(), flingListener.getVelocityX(), flingListener.getVelocityY());
				 }
			}
		});
			...
			public boolean onSingleTapConfirmed(MotionEvent e) {
				...
				if(!inZoom) {
					if(onClickListener != null) {
						Log.e("GestureImageViewTouchListener","onSingleTapConfirmed return true" );
						onClickListener.onClick(image);
						return true;
					}
				}

				Log.e("GestureImageViewTouchListener","onSingleTapConfirmed return false" );
				return false;
			}
		});
	...
	public boolean imageCanDragX(){
		return canDragX && !currentXBoundary;
	}
		...
		if(!inZoom) {
			...
			if(!tapDetector.onTouchEvent(event)) {
				if(event.getPointerCount() == 1 && flingDetector.onTouchEvent(event)) {
					Log.e("GestureImageViewTouchListener","startFling()+event.getAction() ="+event.getAction() );
					startFling();
				}
				...
				if(event.getAction() == MotionEvent.ACTION_UP) {
					...
					Log.e("GestureImageViewTouchListener","stopAnimations");
