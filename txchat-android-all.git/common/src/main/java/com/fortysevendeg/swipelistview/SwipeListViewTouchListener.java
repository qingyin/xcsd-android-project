...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    List<PendingDismissData> pendingDismisses = new ArrayList<PendingDismissData>();
     ...
     */
    private void setFrontView(final int downPosition, View frontView) {
        ...
        try{
        	...
        	Log.i("SwipeListViewTouchListener:",""+e.getMessage());
        }
    }
     ...
     * @param backView
     */
    private void setBackView(final int downPosition, View backView) {
    ...
    protected void dismiss2(int position){
    	int start = swipeListView.getFirstVisiblePosition();
        int end = swipeListView.getLastVisiblePosition();
        View view = swipeListView.getChildAt(position - start);
    ...
    private void closeAnimate(View view, int position) {
        if (opened.get(position) && view != null) {
        	Log.d("SwipeListView", "opened closeAnimate position="+position);
            generateRevealAnimate(view, true, false, position);
        }
    }
    ...
    private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
        	Log.d("swipe", "generateRevealAnimate opened.get(position)="+opened.get(position));
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
            }
        } else {
        ...
        Log.d("swipe", "generateRevealAnimate moveTo="+moveTo+" rightOffset="+rightOffset+" leftOffset="+leftOffset);
    ...
    private void resetCell(View frontView, int downPosition) {
    	Log.e("swipe", "resetCell downPosition="+downPosition);
        if (downPosition != ListView.INVALID_POSITION && frontView != null) {
//            if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
//                backView.setVisibility(View.VISIBLE);
//            }
            Log.e("swipe", "resetCell opened.get(downPosition)="+opened.get(downPosition));
...
//            backView = null;
        ...
        return new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView absListView, int scrollState) {
                ...
                if (swipeClosesAllItemsWhenListMoves && scrollState == SCROLL_STATE_TOUCH_SCROLL) {
                	Log.d("swipe", "scrollState == SCROLL_STATE_TOUCH_SCROLL");
                    closeOpenedItems();
                }
    ...
    void closeOpenedItems() {
    	Log.d("swipe", "closeOpenedItems ");
        if (opened != null) {
            int start = swipeListView.getFirstVisiblePosition();
            int end = swipeListView.getLastVisiblePosition();
            for (int i = start; i <= end; i++) {
                if (opened.get(i)) {
                	Log.d("swipe", "closeOpenedItems= "+i);
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
            	swipeListView.onActionDown(view, motionEvent);
                if (paused && mDownPosition != ListView.INVALID_POSITION) {
                    return false;
                }
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    Log.e("swipe", "childPosition="+childPosition+" allowSwipe="+allowSwipe+"getItemViewType="+swipeListView.getAdapter().getItemViewType(childPosition));
                    if (allowSwipe && rect.contains(x, y)) {
                        ...
                        setFrontView(mDownPosition,child.findViewById(swipeFrontView));

                        frontView.setClickable(!opened.get(mDownPosition));
                        frontView.setLongClickable(!opened.get(mDownPosition));

                        velocityTracker = VelocityTracker.obtain();
                        velocityTracker.addMovement(motionEvent);
                        if (swipeBackView > 0) {
                            setBackView(mDownPosition,child.findViewById(swipeBackView));
                        }
                        break;
                    }
                }
                ...
                Log.e("swipe", "MotionEvent.ACTION_DOWN: return true");
                return true;
            }

            case MotionEvent.ACTION_UP: {
                if (velocityTracker == null || !swiping || mDownPosition == ListView.INVALID_POSITION) {
                	Log.d("swipe", "MotionEvent.ACTION_UP swiping="+swiping+" velocityTracker="+velocityTracker+" downPosition="+ListView.INVALID_POSITION);
                    break;
                }
                ...
                Log.e("swipe", "MotionEvent.ACTION_UP velocityX="+velocityX+" velocityY="+velocityY);
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity && velocityY < velocityX) {
                    ...
                    if (swapRight != swipingRight && swipeActionLeft != swipeActionRight) {
                        swap = false;
                    } else if (opened.get(mDownPosition) && openedRight.get(mDownPosition) && swapRight) {
                        swap = false;
                    } else if (opened.get(mDownPosition) && !openedRight.get(mDownPosition) && !swapRight) {
                        swap = false;
                    } else {
                    	if(MessageFragment.swipFlag)
                        swap = true;
                    }
                ...
                generateAnimate(frontView, swap, swapRight, mDownPosition);
                if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
                	Log.d("SwipeListView", "swapChoiceState downPosition="+mDownPosition);
                    swapChoiceState(mDownPosition);
                }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                if (velocityTracker == null || paused || mDownPosition == ListView.INVALID_POSITION) {
                    break;
                }
                ...
                Log.e("swipe", "MotionEvent.ACTION_MOVE velocityX="+velocityX+" velocityY="+velocityY);

                float deltaX = motionEvent.getRawX() - downX;
                float deltaMode = Math.abs(deltaX);

                int swipeMode = this.swipeMode;
                int changeSwipeMode = swipeListView.changeSwipeMode(mDownPosition);
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    if (opened.get(mDownPosition)) {
                        swipeListView.onStartClose(mDownPosition, swipingRight);
                        ...
                        swipeListView.onStartOpen(mDownPosition, swipeCurrentAction, swipingRight);
                    }
                ...
                if (swiping && mDownPosition != ListView.INVALID_POSITION) {
                    if (opened.get(mDownPosition)) {
                        deltaX += openedRight.get(mDownPosition) ? viewWidth - rightOffset : -viewWidth + leftOffset;
                    }
                    Log.i("haoswip", MessageFragment.swipFlag+"");
                    if(MessageFragment.swipFlag)
                    move(mDownPosition,deltaX);
                    return true;
                }
                break;
            }
     ...
     * @param deltaX delta
     */
    public void move(int downPosition, float deltaX) {
        ...
        if (opened.get(downPosition)) {
            ...
            Log.d("SwipeListView", "opened.get(downPosition) posX="+posX+" viewWidth="+viewWidth+" ");
        }
        if (posX > 0 && !swipingRight) {
            Log.d("SwipeListView", "change to right posX="+posX);
        ...
        if (posX < 0 && swipingRight) {
            Log.d("SwipeListView", "change to left posX="+posX);
            swipingRight = !swipingRight;
