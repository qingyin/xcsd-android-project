...
public class FFMpegUtils {
	...
	public static boolean importVideo(MediaObject part,int videoWidth, int videoHeight, int cutWidth, int cutHeight, String start, String duration, boolean isCrop, int cropX, int cropY) {
		if(part != null && !StringUtils.isEmpty(part.tempPath)) {
			File f = new File(part.tempPath);
			if(f != null && f.exists() && !f.isDirectory()) {
				StringBuffer buffer = new StringBuffer("ffmpeg");
				...
				int var23;
				if(DeviceUtils.hasJellyBeanMr1()) {
					MediaMetadataRetriever hasRotation = new MediaMetadataRetriever();
					hasRotation.setDataSource(part.tempPath);

					try {
						var23 = Integer.parseInt(hasRotation.extractMetadata(24));
					} catch (NumberFormatException var20) {
						var23 = -1;
					}
				} else {
					var23 = UtilityAdapter.VideoGetMetadataRotate(part.tempPath);
				}

				if(var23 == 90 || var23 == 270) {
					width = videoHeight;
					height = videoWidth;
				...
				if(width>(height*4/3)){
					cutHeight = height;
					cutWidth = cutHeight*4/3;
				}else if(width<=(height*4/3)){
					cutWidth = width;
					cutHeight = width*3/4;
				}

				part.setOutputWidth(cutWidth);
				part.setOutputHeight(cutHeight);
				...
				if(isCrop){
					buffer.append(String.format(" crop=%d:%d:",cutWidth,cutHeight));
					buffer.append(cropX);
					buffer.append(":");
					buffer.append(cropY);
				}else {
					buffer.append(String.format(" scale=%d:%d",cutWidth,cutHeight));
				}
				buffer.append("\"");
				if(var25) {
					buffer.append(" -metadata:s:v rotate=\"\"");
				...
				buffer.append(String.format(" -ss %s",start));
				buffer.append(String.format(" -t %s",duration));
				buffer.append(String.format(" -an -vcodec rawvideo -f rawvideo -s %dx%d -pix_fmt yuv420p -r 15 \"",cutWidth,cutHeight));
				buffer.append(part.mediaPath);
				buffer.append("\"");
				...
				boolean var27 = UtilityAdapter.FFmpegRun("", buffer.toString()) == 0;
				if(!var27) {
					VCamera.copyFFmpegLog(buffer.toString());
				}
	 ...
	 * @return
	 */
	public static boolean videoTranscoding(MediaObject mMediaObject, String targetPath) {
		...
		buffer.append(mMediaObject.audioPath);
		buffer.append("\"");
		// 添加视频
		buffer.append(String.format(" -f rawvideo -pix_fmt yuv420p -s %dx%d -r 15 -i \"",mMediaObject.getOutputWidth(),mMediaObject.getOutputHeight()));
		buffer.append(mMediaObject.mediaPath);
		buffer.append("\"");
